...
# 使用Unix shell风格的通配符匹配符合特定格式的文件和文件夹
# 注意：glob默认不匹配以点符号（.）开始的文件
#
# 通配符    功能
# --------------------------------------------------------
# *	        匹配0或多个字符
...
# [!exp]    匹配不在指定范围内的字符
# ------------------
glob.glob(pathname, recursive=False)     # 返回一个符合条件的路径的字符串列表
glob.iglob(pathname, recursive=False)    # 返回一个迭代器
